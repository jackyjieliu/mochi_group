...
- 固定團時間是選填的主揪手寫文字，不使用一般活動的時間選擇器；空白時顯示 `^_^//`。

### 固定團

- schema 版本低於 2 時，顯示但停用「固定團（需更新）」，不可允許寫入缺欄位的舊表格。
- 固定團使用 `isFixedGroup` 與 `fixedTimeText`，時間是主揪選填的手寫文字。
- 固定團在列表置頂。
...
- 星空效果**只套在摺疊標題列**（`.card-summary-bar`），展開後的內容維持一般白底樣式。背景使用 `assets/wish-starfield.svg` 的星芒、星座線與景深小星點，加上低頻流星動畫；不要改回單純 radial-gradient 圓點。標題列子元素要墊到 `z-index: 1`，否則會蓋掉星空。
- `prefers-reduced-motion: reduce` 時保留靜態星空、關閉星光呼吸與流星動畫。
...
- 留言入口同時放在首頁活動卡片與活動詳情的「活動完整紀錄」標題旁，讓使用者不必先進詳情也能留言。
- 活動紀錄中的留言使用獨立綠色訊息卡、左側色條與「留言」文字標籤突出顯示；仍依原本時間順序排列，不要改成置頂而破壞操作紀錄的時間脈絡。
...
- 一般活動與固定團都能留言；首頁卡片與活動詳情都要保留留言入口。
...
成員自助簽到的公開名冊，資料在 `Members` 工作表，入口是篩選列的「打卡」按鈕，開啟 `#modal-checkin`。

三個刻意的產品決策，**不要自作主張改掉**：

1. **不保留歷史。** 每人只有一個 `checkedInAt`，非空字串代表本輪打卡成功。沒有月份欄、沒有出席率、沒有 CheckInLog 表。要做歷史統計是另一個功能，不是「補完」這個功能。
2. **名單所有人都看得到**，包含每個人的狀態。這是公開簽到板，不是管理員專用報表。
3. **不做換月自動重置。** 狀態會一直留著，只有管理員按「重置全部打卡」才會清空。不要新增每月觸發器。

**打卡分兩層，兩層的權限不同，不可合併：**

| 層 | 動作 | 誰能做 | 資料 |
|---|---|---|---|
| 第一層 | 登記打卡 | 任何已登入成員（只能對自己） | 新增名單列，寫 `joinedAt`，`checkedInAt` **留空** |
| 第二層 | 勾選打卡成功 | **只有管理員** | 寫入／清空 `checkedInAt` |

使用者**不能自己標記打卡成功，也不能自己退出名單**——登記就定案，要移除只能由管理員操作。`checkIn_` 絕對不可以自己寫 `checkedInAt`，重新登記也不可覆蓋管理員已經勾選的結果（stub 測試有守這一條）。

其他不變條件：

- 首次打卡自動成為名單成員（寫入 `joinedAt`），之後 `joinedAt` 不可被覆蓋；`displayName` 與 `pictureUrl` 每次打卡都更新，因為 LINE 顯示名稱會變。
- 主按鈕只有兩種狀態：未登記顯示「登記打卡」（綠、可按），已登記顯示「已登記」（灰、disabled）。`disabled` 在這裡是狀態文字不是停用動作，所以要蓋掉 `.line-btn:disabled` 的 opacity，否則字會糊掉。
- 管理員在名單每一列用**勾選框**標記打卡成功（`adminSetCheckIn`，帶 `checked` 布林），旁邊是「移除」。勾選框失敗時要把 checked 還原，因為 DOM 已經先被瀏覽器改掉了。
- `checkIn_` **只認 `payload.actorUserId`**，不接受目標 userId，否則任何人都能替別人登記。
...
- 打卡視窗底部有 `.modal-footer`，放「重置全部打卡」（僅管理員，`display` 控制）與「關閉視窗」，與活動詳情視窗同一個版型。沒有這個 footer 時整個視窗看起來像一個分頁而不是彈出視窗。
- 打卡視窗的排序與篩選是**兩件不同的事，不要混回同一個控制項**：下拉只管名稱（A→Z／Z→A），有沒有打卡成功由三顆 pill 篩選。曾經把「未打卡優先」塞進排序下拉，與 pill 的語意重疊。
- 「@ 標記」複製的是**目前篩選後可見的成員**（受搜尋、pill、排序影響），沿用活動視窗 `btn-mention-attendees` 的作法：只複製 `@名字`，用 `safeCopyText`。
...
- 個人日曆行程預設長度為 2 小時，帶入活動名稱、時間、地點、費用、主揪、說明與活動頁面連結，不帶入參與者名單。固定團沒有可解析的日期時間，不顯示這顆按鈕。
...
- 前端「日曆」按鈕位於篩選列最右（排序 → 打卡 → 宣傳 → 日曆），開啟嵌入式日曆與訂閱連結，關閉時要卸載 iframe。
...
頂部只有一顆「排序」按鈕，點開展開 `#sort-panel`，面板裡有三組控制項：

1. **活動時間**：由近到遠（預設）／由遠到近，**單選互斥**，狀態存在 `eventSortOrder`。固定團沒有日期，兩個方向都永遠釘在最上面，方向只套用在有日期的活動。
2. **顯示分類**：揪團中／固定團／許願團，是**獨立開關**，不是分頁：可以全開、全關，或任意組合，顯示的是所有開啟分類的聯集，狀態存在 `activeFilters` 這個 Set，預設三個全開。**不要改回單選分頁**——搬進面板不改變這一點。
3. **重新整理**：手動同步，`refreshDataFromCloud()`。

分類互斥且完整涵蓋：固定團看 `isFixedGroup`、許願團看 `isWish`、揪團中是兩者皆非的一般活動。

**畫面上有三組不同的 pill 共用 `.tab-btn` 樣式**：排序（`[data-event-sort]`，單選）、分類（`[data-filter]`，多選）、打卡視窗的狀態篩選（`[data-checkin-filter]`，單選）。綁定事件時**一律用 data 屬性選取，絕對不要用 `.tab-btn`**——用了就會把三組綁在一起，主畫面篩選當場壞掉。

篩選列是四顆等寬按鈕（`.nav-action`，`flex: 1 1 0`）填滿整列：**排序／打卡／宣傳／日曆**。配色沿用專案既有的「淺色底 + 飽和文字」慣例（同 `.btn-edit`／`.btn-calendar`／`.btn-delete`）：排序石板灰、打卡藍、宣傳橘、日曆紫；排序展開時整顆反白成深石板色。四顆文字都必須是兩個字，變長會把整列擠壞——所以打卡在後端未就緒時只改 `disabled` 與 `title`，不改文字。

**右下角浮動按鈕只留「揪團」一顆。** 曾經放到四顆，遮掉手機約四分之一的螢幕高度；次要入口一律放篩選列，不要再往 FAB 加。

## 同步、更新與快取

- 前端會定期同步 GAS 資料與檢查 `version.json`，並使用 `_ts` 與 `cache: "no-store"` 降低 LINE 內建瀏覽器快取影響。
- **「網頁很慢」的第一嫌疑犯永遠是併發請求數，不是程式效率。** 2026-08-12 實測：對同一個部署同時送 6 個請求，每個請求的腳本內部只跑 1.2～2.1 秒（開試算表 0.33 秒、組資料 0.50 秒、快取全部命中），但 HTTP 各自等了 25～77 秒。部署設定是「以我的身分執行 + 任何人匿名存取」，所有使用者都算同一個執行身分，Apps Script 會把請求排隊。
- 所以**唯一有效的槓桿是減少請求次數**：`DATA_SYNC_INTERVAL_MS` 60 秒 + 每次 ±25% 隨機抖動（避免所有人對齊在同一秒）。不要為了「即時感」把它調回十幾秒，那是把整個群組一起拖慢。切回前景、重新連線與自己的寫入都會另外立刻同步，日常體感不會因為 60 秒變差。
- 診斷慢的時候不要憑猜測改程式。做法：建立一個**臨時的獨立部署**（新網址、不影響正式 `/exec`），在 `doGet` 加上 `?diag=1` 回傳各步驟毫秒數，量完刪除部署並還原程式碼。輸出只能是耗時與表格維度，不得含活動或成員資料。判讀關鍵：腳本內部毫秒數正常但 HTTP 很久 → 塞車，改程式沒有用。
- GAS `doGet` 命中 Script Cache 就直接回傳，沒有才讀 Sheet；TTL 一小時，因單鍵上限切成 20,000 字元一段。任何成功寫入、歸檔、許願結算或初始化都呼叫 `refreshPublicPayloadCacheQuietly_(spreadsheet)` 當場重建，不是只清除。快取的價值是讓每次執行短一點（約省 1 秒）、隊列消化快一點，**不要期待它解決塞車**。
- `warmPublicPayloadCache` 每 15 分鐘保溫一次：快取還在就立刻結束，只有真的不見了才重建。改 schema 或新增觸發器後要重跑 `setupProject()`。
- 前端讀寫逾時不可放大到幾十秒。逾時值一大，塞車時使用者就會盯著空白等將近一分鐘；放棄後等下一輪同步反而更快。

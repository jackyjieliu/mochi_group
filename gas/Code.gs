 ...
 *
 * 實測（2026-08-12，13 個活動）：開啟試算表 0.33 秒、讀全表加組資料 0.50 秒、
 * 寫快取 0.13 秒、讀快取 0.06 秒。所以重建本身很便宜，快取省下的是約 1 秒。
 *
 * ponytail: 快取不是效能瓶頸的解答。真正的瓶頸是 Apps Script 對併發請求排隊
 * （同一部署 6 個請求同時進來：腳本各自只跑 1.2～2.1 秒，HTTP 卻要等 25～77 秒）。
 * 要讓網頁順，唯一有效的槓桿是「減少請求次數」，看前端 DATA_SYNC_INTERVAL_MS。
 * 這裡的快取只是讓每次執行短一點、隊列消化快一點，不要指望它解決塞車。
 */

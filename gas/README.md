...
這套 GAS 使用「軟刪除 + 歸檔」兩段式：刪除活動時**當下不移除 Sheet 列**，只把 `status` 改為 `deleted`；之後由每日歸檔流程把該列搬到 `EventsArchive` 再從 `Events` 移除。一般讀取只回傳仍需顯示的活動，所以手機、電腦與其他使用同一支 GAS 的網站都不會看到已刪除，或已超過 24 小時保留期的過期活動。
...
| `doGet` 過濾 | 已刪除與已超過 24 小時保留期的過期活動不回傳給前端。 |
| 每日自動歸檔 | 凌晨 4 點把已刪除與已超過保留期的過期活動搬到 `EventsArchive`，並從 `Events` 移除。 |
| 許願午夜結算 | 每日 00 點結算許願活動，未達標者整列刪除且**不歸檔**。 |
| 快取保溫 | 每 15 分鐘檢查公開活動快取，不見了才重建（`warmPublicPayloadCache`）。 |

一般活動在**開始時間 + 2 小時**後標示為「已過期」並停止報名，但會繼續留在網站 24 小時；也就是開始後約 26 小時才從網站隱藏，並在下一次每日歸檔時搬到 `EventsArchive`。
...
| `actorName` | 操作者名稱 |
| `actionType` | `create`、`join`、`leave`、`update`、`handoff`、`delete`、`comment`、`highlight`、`unhighlight`、`admin_remove_attendee`、`checkin`、`admin_check`、`admin_uncheck`、`admin_remove_member`、`admin_reset_checkins` |
| `action` | 可閱讀的操作說明 |
| `details` | 補充 JSON 資料 |
...
| action | 權限 | 行為 |
|---|---|---|
| `checkIn` | 任何已登入成員 | **第一層：登記。** 名單裡沒有就新增一列（寫 `joinedAt`，`checkedInAt` 留空）；已有就只更新顯示名稱與頭像，**不碰 `checkedInAt`** |
| `adminSetCheckIn` | 僅管理員 | **第二層：確認。** 依 `checked` 布林寫入或清空該成員的 `checkedInAt` |
...
已刪除與已超過保留期的過期活動只能在 `EventsArchive` 工作表內查看。`?includeDeleted=1` 參數已移除，不要重新加回 —— 它會讓任何拿到 `/exec` 網址的人取得已刪除活動的完整成員名單。
